...
Test pattern examples for different frameworks and testing scenarios.
...
- **Arrange**: Create test data, set up mocks, configure environment, initialize state.
- **Act**: Call the function, trigger the event, mount the component.
- **Assert**: Check return values, verify mock calls, validate state changes.

Keep sections separated with blank lines or comments. For simple tests, inline is fine. For complex tests, use explicit `// Arrange`, `// Act`, `// Assert` comments.
