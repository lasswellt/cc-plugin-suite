...
Package manager command variants, license classification tables, health score formula, and report templates for the dep-health skill.
...
Parse accordingly based on the detected package manager.
...
|---------|---------|-------|
| MIT License | `MIT` | Most common open-source license. No restrictions for any use. |
| Apache License 2.0 | `Apache-2.0` | Includes patent grant. Requires attribution and notice of changes. |
| BSD 2-Clause | `BSD-2-Clause` | Minimal restrictions. Attribution required. |
| BSD 3-Clause | `BSD-3-Clause` | Like 2-Clause plus non-endorsement clause. |
| ISC License | `ISC` | Functionally equivalent to MIT. |
| Zero-Clause BSD | `0BSD` | No requirements at all. |
...
| Zlib License | `Zlib` | Permissive, similar to MIT. |

### Weak Copyleft Licenses (Review Required)

| License | SPDX ID | Notes |
|---------|---------|-------|
| LGPL 2.1 | `LGPL-2.1` | Copyleft applies to modifications of the library itself, not to code that uses it. Generally safe for npm dependencies used as-is. |
| LGPL 3.0 | `LGPL-3.0` | Same as LGPL-2.1 with additional provisions. Safe for unmodified npm dependency use. |
| Mozilla Public License 2.0 | `MPL-2.0` | File-level copyleft. Modifications to MPL files must be shared, but your own files are unaffected. |
| Eclipse Public License 1.0 | `EPL-1.0` | Similar to MPL. Module-level copyleft. |
| Eclipse Public License 2.0 | `EPL-2.0` | Updated EPL with secondary license option. |
| Common Development and Distribution License | `CDDL-1.0` | File-level copyleft similar to MPL. |

### Strong Copyleft Licenses (Flag for Legal Review)

| License | SPDX ID | Risk | Notes |
|---------|---------|------|-------|
| GPL 2.0 | `GPL-2.0` | High | Derivative works must also be GPL. Using as a dependency may trigger copyleft obligations depending on linking. |
| GPL 3.0 | `GPL-3.0` | High | Same as GPL-2.0 with additional provisions (anti-tivoization, patent grants). |
| AGPL 3.0 | `AGPL-3.0` | Very High | Network use triggers copyleft. If your application uses an AGPL library and is accessible over a network, you may need to release your source code. |
| Server Side Public License | `SSPL-1.0` | Very High | MongoDB's license. Extremely broad copyleft for offering as a service. |
| European Union Public License | `EUPL-1.1` | High | Strong copyleft with broad scope. |

### Special Cases

| License | SPDX ID | Action |
|---------|---------|--------|
| No license detected | `UNKNOWN` | Flag — may be proprietary. Check the package's repository for a LICENSE file. |
| `UNLICENSED` | `UNLICENSED` | Flag — explicitly proprietary. Should not be used unless you have a separate agreement. |
| Dual license | `(MIT OR GPL-3.0)` | Use the more permissive option (MIT in this case). |
| Custom license | N/A | Flag for manual review. Read the license text in the package. |
...
|-------------|-------|---------------|
| 90-100 | A | Excellent — dependencies are well-maintained and secure |
| 80-89 | B | Good — minor issues to address in next maintenance window |
| 70-79 | C | Fair — notable issues that should be addressed soon |
| 60-69 | D | Poor — significant security or compliance risks |
...
Use this template for the full dependency health report written to `${SESSION_TMP_DIR}/dep-health-report.md`:
...
> If all up to date: "All dependencies are up to date."
...
> Section included only when mode=upgrade was used.
