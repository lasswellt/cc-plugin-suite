...
Sprint-review enforces this snippet's presence as a **BLOCKER** via Phase 3.6 Invariant 5: any Agent() prompt template that omits the canonical OUTPUT STYLE snippet causes the sprint to fail until the gap is closed. As of Sprint 3, all 7 UNSAFE reference.md files under `skills/*/reference.md` carry the snippet; Sprint 5 upgrades the enforcement clause accordingly. Previous WARNING-only semantics are retired.
